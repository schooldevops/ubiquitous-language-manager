...
interface AliasApi {
    ...
    @Operation(
        tags = ["Alias",],
        summary = "용어 별칭 전체 교체",
        operationId = "replaceTermAliases",
        description = """해당 용어의 모든 별칭/금지어를 요청 본문 목록으로 교체한다(기존 항목 삭제 후 재등록).""",
        responses = [
            ApiResponse(responseCode = "200", description = "별칭 전체 교체 성공", content = [Content(array = ArraySchema(schema = Schema(implementation = TermAlias::class)))]),
            ApiResponse(responseCode = "400", description = "요청 형식 또는 validation 오류", content = [Content(schema = Schema(implementation = ErrorResponse::class), examples = [
                ExampleObject(name = "invalidRequest", value = "{\n  \"code\": \"INVALID_REQUEST\",\n  \"message\": \"\uC694\uCCAD \uAC12\uC774 \uC62C\uBC14\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4.\",\n  \"detail\": \"koreanName\uC740 \uD544\uC218\uC785\uB2C8\uB2E4.\",\n  \"traceId\": \"01HX8R7P9Q6RZC6Q9VQ0X7Z3WB\"\n}")
            ])]),
            ApiResponse(responseCode = "401", description = "인증 필요", content = [Content(schema = Schema(implementation = ErrorResponse::class), examples = [
    ...
    fun replaceTermAliases(@Parameter(description = "용어 식별자", required = true) @PathVariable("termId") termId: kotlin.String,@Parameter(description = "", required = true) @Valid @RequestBody termAliasCreateRequest: kotlin.collections.List<TermAliasCreateRequest>): ResponseEntity<List<TermAlias>> {
        return ResponseEntity(HttpStatus.NOT_IMPLEMENTED)
    }
}
