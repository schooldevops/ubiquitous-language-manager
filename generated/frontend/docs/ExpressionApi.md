...
|[**replaceTermExpressions**](#replacetermexpressions) | **PUT** /terms/{termId}/expressions | 용어 표현 매핑 전체 교체|
...
# **replaceTermExpressions**
> Array<TermExpression> replaceTermExpressions(termExpressionCreateRequest)
...
let termId: string; //용어 식별자 (default to undefined)
let termExpressionCreateRequest: Array<TermExpressionCreateRequest>; //

const { status, data } = await apiInstance.replaceTermExpressions(
    termId,
    termExpressionCreateRequest
);
```
...
|Name | Type | Description  | Notes|
|------------- | ------------- | ------------- | -------------|
| **termExpressionCreateRequest** | **Array<TermExpressionCreateRequest>**|  | |
| **termId** | [**string**] | 용어 식별자 | defaults to undefined|
