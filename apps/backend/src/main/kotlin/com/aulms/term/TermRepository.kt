...
class TermRepository {
    ...
    fun get(termId: String): Term = record(termId).toTerm()
    ...
    fun listExpressions(termId: String): List<TermExpression> = record(termId).expressions.toList()
    ...
    fun listAliases(termId: String): List<TermAlias> = record(termId).aliases.toList()
    ...
    fun searchDocuments(): List<TermSearchDocument> = terms.values.map { it.toSearchDocument() }

    fun relationshipDocuments(): List<TermRelationshipRecord> = relationships.toList()
